...
export async function canCustomerAccessInquiry(inquiryId: string, customerUserId: string) {
  const rows = await prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`
    SELECT id FROM public.customer_inquiries WHERE id = ${inquiryId} AND "customerUserId" = ${customerUserId} LIMIT 1
  `)
