...
async function getVerifiedSessionUser({ fresh = false }: { fresh?: boolean } = {}): Promise<SessionUser | null> {
  if (!fresh) {
    ...
    return verifyPortalSession(cookieMap.get(PORTAL_SESSION_COOKIE_NAMES.storefront))
  }
  ...
  try {
    ...
    console.warn("[storefront.auth] Falling back to signed session cache", error)
  }
